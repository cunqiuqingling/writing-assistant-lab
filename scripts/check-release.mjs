...
check(styles.includes('0.8.2-R1 · desktop-class iPad workspace and folder management'), 'iPad must keep a desktop-class responsive workspace');
check(styles.includes('@media (min-width: 900px) and (max-width: 1260px)'), 'iPad landscape must keep the three-column workspace');
check(styles.includes('@media (min-width: 768px) and (max-width: 899px)'), 'iPad portrait must keep a two-column desktop-style workspace');
check(workspace.includes('createFolderFromMove'), 'the material move dialog must support creating a destination folder');
check(workspace.includes('workspaceFolderManageModal'), 'custom folders must expose a management dialog');
check(workspace.includes('workspaceFolderDeleteBtn'), 'custom folders must expose an explicit delete action');
check(workspace.includes('folder-tile-manage'), 'custom child-folder tiles must expose a management button');
check(workspace.includes('PROTECTED_SYSTEM_FOLDER_IDS'), 'only structural root folders may remain protected');
check(workspace.includes('hiddenDefaultFolderIds'), 'removable default folders must persist local hidden-state tombstones');
check(workspace.includes('canManageFolder(folder)'), 'default leaf folders must expose folder management');
check(workspace.includes('restoreDefaultFoldersBtn'), 'the library must expose a way to restore removed default folders');
check(workspace.includes('删除默认文件夹'), 'default leaf-folder removal must use explicit user-facing language');
check(styles.includes('0.8.2-R1 · removable default leaf folders'), 'default-folder removal controls must have dedicated styling');
