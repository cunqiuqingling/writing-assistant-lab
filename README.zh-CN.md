...
**核心练习功能永久免费开放：无需注册、无需订阅、无广告，也无需配置AI即可完整使用。** 可选AI解析采用BYOK；使用者既可以选择提供免费API、免费额度或免费模型的服务商，也可以使用付费API。免费范围、额度、地区限制和可用模型由服务商决定，并可能随时调整。
...
使用者并不一定需要购买API。只要服务商支持浏览器直连，就可以选择其提供的**免费API、免费额度、免费套餐或免费模型**；也可以根据需要使用付费API。免费方案通常可能存在调用次数、模型、地区或速度限制，具体以服务商官方控制台和最新说明为准。
...
2. 选择服务商预设：OpenAI、DeepSeek、SiliconFlow / 硅基流动、Google Gemini、Anthropic Claude，或自定义OpenAI兼容服务。服务商支持时，可以直接使用免费API、免费套餐或免费模型。
3. 粘贴从对应服务商官方控制台申请的API Key。建议使用低额度、可撤销、只用于本工具的专用密钥。
4. 检查预设自动填写的 **Base URL、Endpoint和模型名称**。预设只负责填写接口格式，模型是否可用、免费额度、收费方式以及模型名称是否变化，应以服务商当前说明为准。
...
普通Writing Assistant备份不会包含API Key。仅标签页保存的密钥会在标签页会话结束后消失；本地加密密码不会上传，也无法由项目维护者找回。部分服务商可能因为浏览器CORS策略而无法直连。根据所选服务商与套餐，API调用可能在免费额度内完成，也可能产生由服务商收取的费用。

[查看完整AI配置与故障排查说明](docs/AI_CONFIGURATION.md)

## 将仿写复制到外部AI获取反馈

Sentence Lab和Paragraph Lab中的“复制…… · AI反馈”会自动整理参考原文、使用者的写作、笔记以及针对当前训练模式的反馈要求。复制后可以直接粘贴到ChatGPT、Gemini、DeepSeek或其他常用AI平台，请其帮助发现逻辑、语法、搭配和自然度问题。

该操作只写入系统剪贴板，不会自动调用Writing Assistant中的API，也不会自动发送学习者写作。只有使用者主动粘贴并发送时，内容才会提交给所选外部平台。站内“AI原文解析”仍然只分析参考原文，不读取学习者写作。

[查看外部AI反馈复制说明](docs/EXTERNAL_AI_FEEDBACK.md)
